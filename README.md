...
- 프론트엔드: <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=HTML5&logoColor=white"> <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=CSS3&logoColor=white"> <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=JavaScript&logoColor=white">

- 백엔드 <img src="https://img.shields.io/badge/java-007396?style=for-the-badge&logo=OpenJDK&logoColor=white"> <img src="https://img.shields.io/badge/springboot-6DB33F?style=for-the-badge&logo=springboot&logoColor=white">
...
>#### 7.1.2. 프로젝트를 시작하며
>- 조가 정해지고 처음으로 회의를 했다. "조장을 뽑지 않아도 되지 않을까?"라는 의견이 오갔다. 나는 협업에 있어 각자 분명한 책임과 역할을 지고 성실하게 프로젝트에 임하려면 조장이 꼭 필요하다고 생각했다. 그리고 나는 조장이 되었다. 조장을 꼭 해야겠다고는 생각하지 않았다. 생각할 게 너무 많았기 때문이다. 하지만 나도 몰랐던 능력이 있었거나 팀원들이 잘 따라와 줘서 그런지, 조장으로서 업무 분담을 위해 의견을 조율할 때 크게 에너지가 들지 않았다.
>#### 7.1.3. 의견조율
>- 프로젝트를 진행하면서 팀원들의 컨디션을 해치지 않기 위해 새로운 아이디어를 제시할 때는 정당성을 부여했다. 모두가 납득할 수 있을 때만 아이디어를 채택했다. 대표적인 아이디어로는 GPT 활용을 추천했다. GPT를 사용하면 정보 검색 시간이 크게 단축되고, 내가 모르는 것이 무엇인지 파악하는 능력이 향상된다. 또한, 그에 따른 정확한 질문을 하는 능력도 크게 늘어난다. 이는 결국 당면한 문제를 어떻게 해결해야 하는지에 대한 중요한 발판이 된다.
>#### 7.1.4. 트러블슈팅
...
### 7.2. 김원
>#### 7.2.1. 진행한 작업
>- 홈 화면에서 상위 리뷰 도서 추천 기능 구현 (HomeController, HomeService)
>    - 홈 화면에 리뷰 수, 별점, 좋아요를 기준으로 상위 3개의 도서를 추천하고 출력
>- 디테일 페이지의 리뷰 목록 출력 및 CRUD 기능 완성 (ReviewController, ReviewService)
>    - 리뷰 작성, 수정, 삭제 등의 CRUD 기능 구현
>- 마이 페이지에서 개인 리뷰 관리 기능 구현 (MyReviewResponseDTO와 ProfileController)
>    - 마이 페이지에서 개인 리뷰를 조회하고 관리하는 기능 추가
>#### 7.2.2. 문제점 및 어려움
>- 객체를 빌더 패턴으로 생성할 때 여러 객체 간의 매핑이 필요해 정확한 필드 매칭에 어려움이 있었음
>- Controller와 Service 간의 역할을 이론적으로 이해했지만, 실제 코드 구현 시 명확히 분리하는 데 어려움을 겪음
>- JSP의 EL 표현식을 JavaScript와 함께 사용할 때 문법적 문제로 시간 소모
>- 비동기 처리를 구현하며 데이터 흐름을 이해하고 관리하는 데 어려움이 있었음
>#### 7.2.3. 성과
>- 각 레이어(DTO, Repository, Service, Controller)의 역할을 이해하고 구조에 맞게 기능을 구현하는 경험을 쌓음
객체 간 매핑과 데이터 흐름에 대한 이해도가 향상됨
>#### 7.2.4. 개선할 점
>- 빌더 패턴 사용 시 필수 필드와 선택 필드를 구분해 유연하게 객체를 생성하는 방법 익히기
>- Controller와 Service의 역할을 명확히 구분하여 구조적으로 코드를 작성하는 연습을 강화할 필요
>- 프론트엔드 작업에 대한 이해도를 높여 전체 프로젝트에서의 흐름을 보다 원활하게 파악할 것
>#### 7.2.5. 향후 목표
>- 프론트엔드를 React로 전환해 기능 확장 고려
>- 각 레이어가 담당해야 할 역할을 명확히 구분해 유지보수가 용이한 코드를 작성
...
>- 회원 Entity를 만들고 데이터베이스에 연동하는 일부터 쉬운일이 아니었다. 무엇을 모르는지 모르는 상태에서 적절한 질문조차 할 수 없었다. 스프링부트 등 편리한 도구들이 개발을 지원하고 있지만 여러 계층으로 따라 데이터가 오고가는 흐름과 그 실현은 혼란스러운 용어들과 함께 이해하기 어려운 일이었다. 방향을 찾기 시작한 것은 이메일 인증을 통한 회원가입 기능을 구현하면서부터다. 메일과 연동하는 설정을 순서대로 진행하고, 입력에서 데이터베이스까지의 회원가입 논리를 구현하면서 가능성을 맛보았다.
>- 비밀번호 찾기 기능도 그 연장에서 구현할 수 있을테지만 시간이 허락하지 않는다. 사이트를 통한 회원가입도 나중에 구현해보고 싶은 마음이다.
...
>- 백엔드의 논리를 생각하면서 종종 프런트를 수정해야 하는 일을 마주하였다. 그러나 HTML과 자바스크립트에 대한 두려움으로 어쩌지 못하는 일이 종종 있었다. 복습의 필요성을 느낀다. 그리고 CRUD 게시판과 동료들이 구현한 기능을 온전히 내 힘으로 구현하고 싶다. 이번 프로젝트는 전체적인 흐름을 이해하고, 배우고 싶은 욕구와 동기를 느끼기에 그 가치가 충분하다. 시간이 허락하는 한 계속 배움을 지속할 것이다.
