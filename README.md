...
>- 회원 Entity를 만들고 데이터베이스에 연동하는 일부터 쉬운일이 아니었다. 무엇을 모르는지 모르는 상태에서 적절한 질문조차 할 수 없었다. 스프링부트 등 편리한 도구들이 개발을 지원하고 있지만 여러 계층으로 따라 데이터가 오고가는 흐름과 그 실현은 혼란스러운 용어들과 함께 이해하기 어려운 일이었다. 방향을 찾기 시작한 것은 이메일 인증을 통한 회원가입 기능을 구현하면서부터다. 메일과 연동하는 설정을 순서대로 진행하고, 입력에서 데이터베이스까지의 회원가입 논리를 구현하면서 가능성을 맛보았다. 비밀번호 찾기 기능도 그 연장에서 구현할 수 있을테지만 시간이 허락하지 않는다.
...
>- 이번 프로젝트는 전체적인 흐름을 이해하고, 배우고 싶은 욕구와 동기를 느끼기에 그 가치가 충분하다. 백엔드의 논리를 생각하면서 종종 프런트를 수정해야 하는 일을 마주하였다. 그러나 HTML과 자바스크립트에 대한 두려움으로 어쩌지 못하는 일이 종종 있었다. 복습의 필요성을 느낀다. 그리고 CRUD 게시판과 동료들이 구현한 기능을 온전히 내 힘으로 구현하고 싶다. 시간이 허락하는 한 계속 배움을 지속할 것이다.
